...
namespace Bot.Modules.Commands
{
    ...
    {
         public static void UserTotalPointsOnThisChannel(StreamsContext db, IrcClient irc, string channel, string user, string msg)
        {
            if (Extensions.CheckIfStreamExists(db, channel))
            {
                ...
                if (userId != -1)
                {
                    if (ConfigParams.Debug)
                        irc.SendPublicChatMessage(channel, $"UserTotalPointsOnThisChannel: {user} was found in this channel.");
                    var stream = db.Streams.Where(x => x.channelName.Equals(channel)).Include(x => x.Users).First();
                    irc.SendPublicChatMessage(channel, $"{user} posiada {stream.Users[userId].TotalPoints} {stream.PointsName}");
                }
                else
                    if (ConfigParams.Debug)
                    irc.SendPublicChatMessage(channel, "UserTotalPointsOnThisChannel: user was not found in this channel.");
            }
            else
            {
                if (ConfigParams.Debug)
                    irc.SendPublicChatMessage(channel, "UserTotalPointsOnThisChannel: stream was not found in this channel..");
            }
        }
        public static void UserTotalPointsOnOtherChannel(StreamsContext db, IrcClient irc, string channel, string sender, string msg, string pointsCommand)
        {
            ...
            if (streams.Count() >= 1)
            {
                ...
                {
                    if (ConfigParams.Debug)
                        irc.SendPublicChatMessage(channel, $"UserTotalPointsOnChannel: {sender} was found in {stream.channelName}.");
                    irc.SendPublicChatMessage(channel, $"{sender} posiada {stream.Users[userId].TotalPoints} {stream.PointsName}");
                }
                else
                    if (ConfigParams.Debug)
                    irc.SendPublicChatMessage(channel, $"UserTotalPointsOnChannel: user was not found in {stream.channelName}.");
            }
            else
            {
                if (ConfigParams.Debug)
                    irc.SendPublicChatMessage(channel, $"UserTotalPointsOnChannel: stream with {pointsCommand} was not found ..");
            }
        }
